...
while(mpciter < sim_tim / T) % new - condition for ending the loop
    ...
    for k = 1:N %new - set the reference to track
        ...
        x_ref = 5*sin(0.1*t_predict); y_ref = 7-5*cos(0.1*t_predict); theta_ref = 0.1*t_predict;
        u_ref = 0.5; omega_ref = 0.1;
        if theta_ref >= 2*pi % the trajectory end is reached
            x_ref = 0; y_ref = 2; theta_ref = 0;
            u_ref = 0; omega_ref = 0;
        end
    ...
    [t0, x0, u0] = shift_Runge_Kutta(T, t0, x0, u,f);
    xx(:,mpciter+2) = x0;
...
for k=1:1:314
    Xout(k,1) = 5 * sin(0.02 * k);     % 参考轨迹 X 坐标
    Xout(k,2) = 7 - 5 * cos(0.02 * k);% 参考轨迹 Y 坐标
end

for k = 1:size(xx,2)
    plot(Xout(:,1), Xout(:,2), '-g','linewidth',1.2); hold on;  % plot the reference trajectory
...
% video = VideoWriter('figure and animation/tt_circle.avi','Motion JPEG AVI');
